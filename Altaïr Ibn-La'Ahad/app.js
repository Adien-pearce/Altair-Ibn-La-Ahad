...
const API_KEY = "AIzaSyAyeI_Z8nIr7evML9WIxWQRypMHoimLeKU";
...
async function saveMessage(sender, text) {
    if (!window.firebaseImports || !window.db || !window.userId) {
        ...
        localStorage.setItem('auri_history', JSON.stringify(auriHistory));
        return;
    }
    ...
    try {
        await addDoc(collection(window.db, auriCollectionPath), {
            sender: sender,
            text: text,
...
document.addEventListener('auth-ready', () => {
    ...
    // Homepage Timer for Login/Join Popup
    startHomepageTimer();
